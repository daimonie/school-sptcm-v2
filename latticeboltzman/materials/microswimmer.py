...
cdx     = nx/2                    # Center of dipole in x
cdy     = ny/2                    # Center of dipole in y
Fdip    = 1.0e-05                 # Dipole force
ldip    = 5.0                     # Dipole length
rblob   = 2.0                     # Extent of the force interpolation
...
for time in range(maxIter):
    ...
    # Add the dipole force onto the fluid
    
    posf = fromfunction(lambda x,y: (x - cdx - ldip/2.0)**2.0 + (y - cdy)**2.0 < rblob**2, (nx,ny))
    negf = fromfunction(lambda x,y: (x - cdx + ldip/2.0)**2.0 + (y - cdy)**2.0 < rblob**2, (nx,ny))

    posweight = float(sum(posf))
    negweight = float(sum(negf))

    u[0,posf] += tau*(Fdip/posweight)/rho[posf]
    u[0,negf] -= tau*(Fdip/posweight)/rho[negf]
    ...
    if ( time%nSnap == 0 ):
        ...
        plt.xlabel("x")
        plt.ylabel("y")
        plt.title("Stokes Flow around a Dipole Microswimmer")
        plt.savefig("microswimmer_vel_"+str(time/nSnap).zfill(1)+".png")

    # Export the flow field for post-processing
    if ( time%nSnap == 0 ):
        file = open("microswimmer_flow_field_"+str(time/nSnap).zfill(1)+".dat","w")
